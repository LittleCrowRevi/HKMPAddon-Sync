...
namespace HKMPAddon.HKMP
{
    ...
    /// </summary>
    internal class HKMPAddonServerAddon : ServerAddon
    {
        ...
        public override void Initialize(IServerApi serverApi)
        {
            ...
            (
                ...
                (id, packetData) =>
                {
                    HkmpAddon.Instance.Log($"[Server] Message from client {id}: {packetData.Message}");
            ...
            {
                HkmpAddon.Instance.Log($"[Server] Player of ID {player.Id} has connected to the server.");
            };

            serverApi.ServerManager.PlayerDisconnectEvent += (player) =>
            {
                HkmpAddon.Instance.Log($"[Server] Player of ID {player.Id} has disconnected to the server.");
            };

            serverApi.ServerManager.PlayerEnterSceneEvent += (player) =>
            {
                HkmpAddon.Instance.Log($"[Server] Player of ID {player.Id} has entered a scene.");
            };

            serverApi.ServerManager.PlayerLeaveSceneEvent += (player) =>
            {
                HkmpAddon.Instance.Log($"[Server] Player of ID {player.Id} has left a scene.");
            };
        }
    }
}